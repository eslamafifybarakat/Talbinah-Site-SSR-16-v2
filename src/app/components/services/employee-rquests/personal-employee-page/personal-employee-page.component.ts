// Modules
import { ActivatedRoute, Router, RouterModule } from '@angular/router';
import { TranslateModule } from '@ngx-translate/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';

//Services
import { MetadataService, MetaDetails } from './../../../../services/generic/metadata.service';
import { EmployeeRequestsService } from './../../../../services/employee-requests.service';
import { environment } from './../../../../../environments/environment';
import { catchError, finalize, tap } from 'rxjs/operators';
import { Subscription } from 'rxjs';

// Components
import { EmployeeRequestCardComponent } from '../employee-request-card/employee-request-card.component';
import { Component } from '@angular/core';

@Component({
  selector: 'app-personal-employee-page',
  standalone: true,
  imports: [
    TranslateModule,
    CommonModule,
    RouterModule,
    FormsModule,

    //Components
    EmployeeRequestCardComponent
  ],
  templateUrl: './personal-employee-page.component.html',
  styleUrls: ['./personal-employee-page.component.scss']
})
export class PersonalEmployeePageComponent {
  private subscriptions: Subscription[] = [];
  private imageBaseUrl: string = '';
  private metaDetails: MetaDetails | undefined;
  private fullPageUrl: string = '';

  /* --- Start Employee Details Variables --- */
  private employeeId: any = null;
  placeDetails: any | undefined;
  isLoadingPlaceDetails: boolean = false;
  /* --- End Employee Details Variables --- */

  /* --- Start Employee Rquests List Section Variables --- */
  isLoadingEmployeeRquestsList: boolean = false;
  employeeRquestsList: any[] = [];
  employeeRquestsCount: number = 0;
  page: number = 1;
  perPage: number = 18;
  /*  --- End Employee Rquests List Section Variables ---  */


  constructor(
    private employeeRequestsService: EmployeeRequestsService,
    private metadataService: MetadataService,
    private activatedRoute: ActivatedRoute,
    private route: Router
  ) { }

  ngOnInit(): void {
    this.initPageData();
  }
  private initPageData(): void {
    this.updateMetaTagsForSEO();
    this.imageBaseUrl = environment.imageBaseUrl;
    this.activatedRoute.params.subscribe((params) => {
      this.employeeId = params['queryParm'];
      if (this.employeeId) {
        this.getEmployeeById(this.employeeId);
        // this.fullPageUrl = environment.publicUrl + this.localizationLanguageService.getFullURL();
      }
    });
    this.getEmployeeRquestsList();
  }
  private updateMetaTagsForSEO(): void {
    let metaData: MetaDetails = {
      title: 'طلبات الإستقدام',
      description: 'الوصف',
      image: 'https://avatars.githubusercontent.com/u/52158422?s=48&v=4'
    }
    this.metadataService.updateMetaTagsForSEO(metaData);
  }

  /* --- Start Employee Details Functions --- */
  getEmployeeById(id: any, preventLoading?: boolean): void {
    if (!preventLoading) {
      this.isLoadingPlaceDetails = true;
    }
    let placeDataSubscription: Subscription = this.employeeRequestsService.getEmployeeRequestItemData(id).pipe(
      tap((res: any) => {
        if (res?.code === 200) {
          this.placeDetails = res.data;
          this.handlePlaceDetails();
        } else {
          this.handleError(res?.message);
        }
      }),
      catchError(err => {
        this.handleError(err);
        return []; // Return an empty array or appropriate fallback value
      }),
      finalize(() => {
        this.isLoadingPlaceDetails = false;
        this.updateMetaTagsForSEO(); // Remove After Function Working Successfully
      })
    ).subscribe();

    this.subscriptions.push(placeDataSubscription);
  }
  private handlePlaceDetails(): void {
    this.updateMetaTagsForSEO();
  }
  /* --- Start Employee Details Functions --- */


  /* --- Start Employee Rquests List Section Functions --- */
  private getEmployeeRquestsList(preventLoading?: boolean): void {
    if (!preventLoading) {
      this.isLoadingEmployeeRquestsList = true;
    }
    let employeeRquestsListSubscription: Subscription = this.employeeRequestsService.getEmployeeRequests(this.page, this.perPage).pipe(
      tap((res: any) => {
        if (res.code == 200) {
          if (res.data) {
            this.employeeRquestsList = res.data.items;
            this.employeeRquestsCount = res.data.total;
          }
        } else {
          this.handleError(res?.message);
        }
      }),
      catchError(err => this.handleError(err)),
      finalize(() => {
        setTimeout(() => {
          this.setDummyData();
          this.isLoadingEmployeeRquestsList = false;
        }, 1000);
      })
    ).subscribe();

    this.subscriptions.push(employeeRquestsListSubscription);
  }
  private setDummyData(): void {
    [1, 2, 3].forEach(element => {
      this.employeeRquestsList.push({
        id: element,
        title: 'محمد أبوتريكة',
        role: 'سائق خاص',
        countryFlag: 'data:image/jpeg;base64,',
        countryName: 'المملكة العربية السعودية',
        job: 'سائق خاص',
        maritalStatus: 'أعزب',
        experienceYears: 5,
        salary: 2000
      });
    });
    this.employeeRquestsCount = 3225;
  }
  goToEmployeeRquestDetails(item: any | null): void {
    let queryKey: string | number | null = item?.id;
    if (queryKey) {
      this.route?.navigate(['/Services/Employee/Details/' + queryKey]);
    } else {
      // Data is not valid
    }
  }
  /* --- End Employee Rquests List Section Functions --- */

  goToReserve():void {
    this.route.navigate(['/Reservation/LoginForOrderCompletetion']);
  }

  /* --- Handle api requests error messages --- */
  private handleError(err: any): any {
    this.setErrorMessage(err || 'An error has occurred');
  }
  private setErrorMessage(message: string): void {
    // Implementation for displaying the error message, e.g., using a sweetalert
    console.log(message);
    // this.alertsService?.openSweetAlert('error', message);
  }

  ngOnDestroy(): void {
    this.subscriptions.forEach((subscription: Subscription) => {
      if (subscription && subscription.closed) {
        subscription.unsubscribe();
      }
    });
  }
}
